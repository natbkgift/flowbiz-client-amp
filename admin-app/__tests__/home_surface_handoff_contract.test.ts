...
describe('home surface handoff contract', () => {
  ...
  it('keeps the home performance hooks for optimized hero and card media', () => {
    ...
    expect(featuredProjects).toContain('mapProjectToPublicCardData');
    expect(featuredProjects).toContain('ProjectCard as PublicProjectCard');
    expect(featuredProjects).toContain('imagePriority={shouldPreloadMedia}');
    expect(featuredProjects).toContain('imageSizes=');
    expect(featuredProjects).toContain('fallbackImageSrc={fallbackImage}');
    expect(featuredProjects).toContain('prefetch={false}');
    expect(featuredProjects).not.toContain('quality={60}');
    expect(featuredProjects).not.toContain('unoptimized={false}');
    expect(featuredProjects).not.toContain('premium-project-card__cta');
