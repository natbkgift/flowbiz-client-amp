...
export async function generateMetadata({
  ...
  return makeListingPageMetadata(locale, 'buy/condo-pattaya', title, desc, dict.brand.name, resolvedSearchParams);
}

export default async function BuyCondoPattayaPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale: rawLocale } = await params;
  const locale = normalizeLocale(rawLocale);
  const dict = getDictionary(locale);
