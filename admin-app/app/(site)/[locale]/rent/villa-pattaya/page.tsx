...
export async function generateMetadata({
  ...
  return makeListingPageMetadata(locale, 'rent/villa-pattaya', title, desc, dict.brand.name, searchParams);
}

export default async function RentVillaPattayaPage({ params }: { params: { locale: string } }) {
  const locale = normalizeLocale(params.locale);
  const dict = getDictionary(locale);
  ...
  try {
    res = await fetchProperties({ type: 'rental', limit: 60, sort: 'newest', search: 'villa' });
  } catch {
