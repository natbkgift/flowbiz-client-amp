...
import { CompareEventTracker } from './CompareEventTracker';
...
export default async function ComparePage({
  ...
  const evals = await Promise.all(
    ids.map(async (id) => {
      try {
        return await fetchProjectEvaluation(id);
      } catch {
        return null;
      }
    })
    ...
    return (
      <main id="main-content">
        <section className="hero hero--page">
          <Container>
            <h1 className="headline">{dict.compare.title}</h1>
            <p className="subhead">{dict.compare.requiresTwo}</p>
          </Container>
        </section>

        <section className="section">
          <Container>
            <div className="trust-box mb-4" role="status" aria-live="polite">
            ...
            <div className="card reveal">
              <h2 className="card-title">{dict.compare.getStarted}</h2>
              <p className="card-subtitle">{dict.compare.getStartedDesc}</p>
              <div className="cta-row">
                <Link
                  className="btn btn-cta"
                  href={withLocale(locale, '/smart-finder')}
                  data-amp-event-type="compare_cta_click"
                  data-amp-event-payload={JSON.stringify({ action: 'go_to_smart_finder' })}
                >
                  {dict.compare.goToSmartFinder}
                </Link>
                <Link
                  ...
                  data-amp-event-type="compare_cta_click"
                  data-amp-event-payload={JSON.stringify({ action: 'browse_projects' })}
                >
                  {dict.compare.browseProjects}
                </Link>
              </div>
  ...
  return (
    <main id="main-content">
      ...
      <section className="section">
        <Container>
          <CompareEventTracker addedIds={items.map((ev) => ev.project.id)} removedIds={missing} />
          {missing.length ? (
          ...
          <div className="card reveal">
            ...
            <div className="cta-row mt-4">
              <Link
                ...
                href={withLocale(locale, '/smart-finder')}
                data-amp-event-type="compare_cta_click"
                data-amp-event-payload={JSON.stringify({ action: 'back_to_smart_finder' })}
              >
              ...
              <Link
                ...
                data-amp-event-type="compare_cta_click"
                data-amp-event-payload={JSON.stringify({ action: 'get_investment_plan' })}
              >
