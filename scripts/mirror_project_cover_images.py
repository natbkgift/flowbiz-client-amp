...
def _print_human_summary(report: dict[str, Any]) -> None:
    ...
    print(
        ...
        f"mirrored={summary.get('mirrored_new_count', 0)} "
        f"reused={summary.get('reused_existing_count', 0)} "
        f"external_remaining={summary.get('external_remaining_count', 0)} "
        f"broken_local={summary.get('local_missing_files_count', 0)}"
    )
