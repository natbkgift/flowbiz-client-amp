...
class LINENotifier:
    ...
    def __init__(self, channel_access_token: Optional[str] = None, group_id: Optional[str] = None):
        """
        ...
        if not self.group_id:
            raise ValueError(
                "LINE_GROUP_ID not set. Please set environment variable or pass as parameter."
            )
    ...
    def send_message(self, message: str) -> bool:
        ...
        """
        try:
            payload = {"to": self.group_id, "messages": [{"type": "text", "text": message}]}
            ...
            with httpx.Client() as client:
                response = client.post(
                    self.API_ENDPOINT, json=payload, headers=headers, timeout=30.0
                )
    ...
    def send_weekly_report(self, performance: SalesPerformance) -> bool:
        ...
        Returns:
            True if successful
        """
        start_text = performance.period_start.strftime("%d/%m")
        end_text = performance.period_end.strftime("%d/%m/%Y")
        week_text = f"{start_text} - {end_text}"
        ...
        flex_content = {
            ...
            "body": {
                ...
                "contents": [
                    ...
                    {"type": "text", "text": "👥 Top Performers", "weight": "bold", "margin": "lg"},
                    *agent_lines,
                    {"type": "separator", "margin": "lg"},
                    {"type": "text", "text": "📱 Top Sources", "weight": "bold", "margin": "lg"},
                    *source_lines,
                ],
                "paddingAll": "15px",
            },
