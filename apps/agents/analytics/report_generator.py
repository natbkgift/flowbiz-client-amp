...
from .schemas.lead import Lead, LeadPriority, LeadStatus
from .schemas.performance import AgentMetrics, SalesPerformance, SourceMetrics, TeamPerformance
...
class ReportGenerator:
    ...
    def generate_sales_performance(
        self, period_start: date, period_end: date, period_name: str = ""
    ) -> SalesPerformance:
        """
        ...
        period_leads = [
            ...
            if lead.date_created and period_start <= lead.date_created <= period_end
        ]
        ...
        performance = SalesPerformance(
            period_start=period_start, period_end=period_end, period_name=period_name
        )
        ...
        performance.warm_leads = len(
            [l for l in all_active_leads if l.priority == LeadPriority.WARM]
        )
        performance.cold_leads = len(
            [l for l in all_active_leads if l.priority == LeadPriority.COLD]
        )

        # Conversion metrics
        performance.converted_leads = len(
            [l for l in period_leads if l.status == LeadStatus.CONVERTED]
        )
        performance.lost_leads = len([l for l in period_leads if l.status == LeadStatus.LOST])
        performance.unqualified_leads = len(
            [l for l in period_leads if l.status == LeadStatus.UNQUALIFIED]
        )

        if performance.new_leads > 0:
            performance.conversion_rate = (
                performance.converted_leads / performance.new_leads
            ) * 100
        ...
        if performance.converted_leads > 0:
            performance.average_deal_size = (
                performance.total_closed_value / performance.converted_leads
            )
        ...
        performance.total_follow_ups_today = len(
            [l for l in all_active_leads if l.next_follow_up and l.next_follow_up == today]
        )
    ...
    def generate_team_performance(self, period_start: date, period_end: date) -> TeamPerformance:
        """
        ...
        # Period leads
        period_leads = [
            l for l in self.leads if l.date_created and period_start <= l.date_created <= period_end
        ]

        team = TeamPerformance(period_start=period_start, period_end=period_end)
        ...
        # Financial metrics
        team.total_pipeline_value = sum(l.expected_value for l in active_leads if l.expected_value)
        team.total_closed_value = sum(l.expected_value for l in conversions if l.expected_value)
        ...
        agents_with_overdues = set(
            l.assigned_agent for l in active_leads if l.is_overdue and l.assigned_agent
        )
        team.agents_with_overdues = len(agents_with_overdues)

        # Top performers
        team.top_agents_by_leads = sorted(
            agent_lead_counts.items(), key=lambda x: x[1], reverse=True
        )[:5]

        team.top_agents_by_conversions = sorted(
            agent_conversions.items(), key=lambda x: x[1], reverse=True
        )[:5]

        # Top sources
        source_counts = self._count_by_field(period_leads, "source")
        team.top_sources = sorted(source_counts.items(), key=lambda x: x[1], reverse=True)[:5]
    ...
    def generate_daily_report_data(self) -> dict:
        ...
        new_yesterday = [l for l in self.leads if l.date_created == yesterday]

        # Today's follow-ups
        today_followups = [l for l in self.leads if l.next_follow_up == today and l.is_active]
    ...
    def _generate_agent_metrics(
        self, period_leads: List[Lead], all_active_leads: List[Lead]
    ) -> List[AgentMetrics]:
        ...
        for agent in agents:
            ...
            metric.total_actual_value = sum(l.expected_value for l in converted if l.expected_value)
    ...
    def _generate_source_metrics(self, period_leads: List[Lead]) -> List[SourceMetrics]:
        ...
        for source in sources:
            ...
            metric.total_value = sum(l.expected_value for l in converted if l.expected_value)
